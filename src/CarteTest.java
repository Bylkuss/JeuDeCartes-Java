...
public class CarteTest {
    ...
    private void testerValiderCouleur() {
        ...
        for (int i = 0; i < couleursValides.length && testReussi ; i++) {
            testReussi = Carte.laCouleurEstValide(couleursValides[i]);
        }
        ...
        testReussi = true;
        for (int i = 0; i < couleursValides.length && testReussi ; i++) {
            testReussi = Carte.laCouleurEstValide(couleursValides[i].toUpperCase());
        }
        afficherResultatDuTest();
        ...
        for (int i = 0; i < couleursInValides.length && testReussi ; i++) {
            testReussi = !Carte.laCouleurEstValide(couleursInValides[i]);
        }
        afficherResultatDuTest();
    }
    ...
    private void testerValiderValeur() {
        ...
        for (int valeur = 1; valeur <= 13 && testReussi; valeur++) {
            testReussi = Carte.laValeurEstValide(valeur);
        }
        afficherResultatDuTest();

        descriptionTest = "Les valeurs inférieures à 1 sont invalides.";
        for (int  = -20;  <= 0 && testReussi; ++) {
            testReussi = !Carte.laValeurEstValide();
        }
        afficherResultatDuTest();
